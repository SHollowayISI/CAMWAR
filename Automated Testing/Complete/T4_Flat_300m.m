...
scenario.environment = struct( ...
    ...
    ... % Channel Properties
    'channel_loss_on',      true, ...      % Whether to include environment
    ...                                     % modeling in LOS channel loss
    'fog_density',          0.05, ...       % 0.05 for medium, 0.5 for thick
    ...
    ... % Terrain Properties
    'gnd_on',               true, ...       % Simulate terrain T/F
    'gnd_res',              5, ...         % Distance between targets
    'gnd_xlim',             [0 2000], ...  % Down-range min-max limits
    'gnd_ylim',             [-2000 2000], ...   % Cross-range min-max limits
    'gnd_zavg',             -300, ...       % Average ground elevation
    'gnd_var',              1, ...         % Ground height variance
...
scenario.radarsetup = struct( ...
    ...
    'f_s',          2*37.5e6, ...             % ADC sample frequency in Hz
    't_ch',         3*31.25e-6, ...              % Chirp duration in seconds
    ...
    'int_start',    774, ...            % Range to start performing range bin combination
    'range_int',    true, ...           % Range combining Y/N
