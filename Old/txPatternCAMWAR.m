function [az_out, el_out, pattern] = txPatternCAMWAR(tilt)
%TXPATTERNCAMWAR Returns Tx antenna pattern for CAMWAR
...
pattern = Gt*(sinc(phi/(0.5*pi*del_phi)).^4).*(sinc((theta-tilt_rad)/(0.5*pi*del_theta)).^2);
